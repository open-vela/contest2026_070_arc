...
- **电子垃圾再利用**：一块被淘汰的平板电脑无资料的屏，实现近乎零新硬件成本的产品化。
...
- **起步方式**：项目不是从"写应用"开始，而是从**点亮一块没有资料的屏**开始——屏幕不亮则后续一切无从谈起；屏驱动由 Skill 从 安卓 DTB 移植而来，是典型的"AI 原生 BSP"。

**研发历程（整体进度，2026-07-22 ~ 09-11）**

项目早期（7-22~7-28）在 `/data/openvela` 工作区完成"从屏到触摸"的底层打通，8 月起迁至 `/data/dm` 进入应用与产品化阶段。全周期约 3 周，578 次编译打包固化，全程使用AGENTS.md 主导 AI Coding（Mimo AtomCode Opencode）。
...
- **开发板选型**：R528S3-Gemini-S1 大会赞助，屏幕是取自RK3399三防平台利旧屏幕，零采购成本；Cortex-A7 + 128MB 恰好承载 LVGL 大屏 + 音频，秒级开机契合桌面常开设备。
...
| 驱动/适配 | 类型 | 难点与解决方案 |
|---|---|---|
| BOE 1200×1920 面板 | 新增 MIPI DSI 面板驱动 | 无 datasheet → 从 Android 5.10 vendor DTB 逆向 init 序列/时序/引脚映射；`CONFIG_BOE_1200X1920_MIPI` 与 `CONFIG_T070S140B_MIPI` 互斥编译（防 `g_lcd0_config` 重定义）；驱动层 `degree0` 实现横屏 |
...
- 开发日志：**79 篇 devlog / 216 个节点（P1–P216）**，全程节点索引可溯（仓 `docs/devlogs/`）。
...
| 项目完整度（20） | 3.5（15 项功能测试逐项结果）；专属仓全量源码（可复现编译烧录）；作品照片/视频（待补） |
| AI 开发（10） | 3.3（AI 算法）；3.6（100% AI 代码 / 149 会话 / 578 固化 / 6 Skill / 4.21 亿 Token / 记忆体系） |
| 商业潜力（10） | 3.7（应用前景与商业价值） |
| 展示效果（10） | 演示视频（待录）；海报；答辩 PPT（待做） |

## 5、注意事项

- 提交截止：2026 年 9 月 20 日。
...
| 9-10 | `9-10part3devlog.md` | P209：用户质疑 WiFi 配置没打进包，二进制抠出 usrdata.fex 内 llh 配置自证包无问题，确认 lunch 2 与 vela.bin==nsh.fex；日历板上翻不了月根因是透明箭头圆钮，改标题大行+文字药丸。 |
